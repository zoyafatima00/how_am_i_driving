...
class AdminScheduleVm extends ChangeNotifier {
  ...
  //get list by name
  Future<void> fetchRideListByName() async {
    _isLoading = true;
    notifyListeners();

    try {
      // Retrieve full_name from SharedPreferences
      String? driverName =
          await MyPrefs.getStringShared(SharedPrefsKeys.FULL_NAME);
      print('Driver name from shared prefs: $driverName');
      if (driverName == null || driverName.isEmpty) {
        throw Exception("Driver name not found.");
      }

      // Pass the driverName to the API call
      ApiResponse apiResponse = await authRepo.getRideListByName(driverName);
